...
function atualizarPreview() {
  ...
  if (resumoProfissional.trim()) {
    html += `<h2>Resumo Profissional</h2><p style="font-size:12px; line-height:1.3;">${resumoProfissional.replace(/\n/g, "<br>")}</p>`;
  }
...
if (experiencias) {
  ...
  const informacoesComplementares = (document.getElementById("informacoesComplementares") || {}).value || "";
  if (informacoesComplementares.trim()) {
    html += `<h2>Informações Complementares</h2><p style="font-size:12px; line-height:1.3;">${informacoesComplementares.replace(/\n/g, "<br>")}</p>`;
  }
...
["nomeCompleto","subtitulo","telefone","email","localizacao","linkedin","portfolio","objetivo","resumoProfissional","informacoesComplementares"].forEach(id => {
...
if (idade || telefone || email || localizacao || linkedin || portfolio || pretensao) {
  ...
  // Resumo Profissional
  const resumoProfissional = (document.getElementById("resumoProfissional") || {}).value.trim() || "";
  if (resumoProfissional) {
    doc.setFontSize(13);
    doc.setFont("helvetica", "bold");
    y = escreverTexto("Resumo Profissional:", 10, 180, y, doc);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    y = escreverTexto(resumoProfissional, 12, 180, y, doc);
    y += 8;
  }
...
if (descricao) {
  ...
  // Informações Complementares
  const informacoesComplementares = (document.getElementById("informacoesComplementares") || {}).value.trim() || "";
  if (informacoesComplementares) {
    doc.setFontSize(13);
    doc.setFont("helvetica", "bold");
    y = escreverTexto("Informações Complementares:", 10, 180, y, doc);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    y = escreverTexto(informacoesComplementares, 12, 180, y, doc);
    y += 8;
  }
}
...
function importarTXT(event) {
  ...
  reader.onload = function(e) {
    ...
    linhas.forEach(linha => {
      ...
      else if (linha.startsWith("Resumo Profissional:")) secaoAtual = "resumoProfissional";
      ...
      else if (linha.startsWith("Informações Complementares:")) secaoAtual = "informacoesComplementares";
      ...
      // Resumo Profissional
      else if (secaoAtual === "resumoProfissional" && linha.trim() !== "") {
        const campo = document.getElementById("resumoProfissional");
...
// Habilidades (compatível com addHabilidade: placeholder="Habilidade")
  ...
  const partes = linha.trim().replace(/^[-•]\s*/, "").split("(");
      ...
      else if (secaoAtual === "informacoesComplementares" && linha.trim() !== "") {
        const campo = document.getElementById("informacoesComplementares");
        campo.value += (campo.value ? "\n" : "") + linha;
      }
    });
